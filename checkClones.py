...
# -------------------------
excel_file = "ZGVTT_GRP_F_TEXT.xlsx"
# excel_file = "ZGVTt_GRP_F_COM.xlsx"
# excel_file = "ZGVTt_GRP_F_POL.xlsx"
# excel_file = "ZGVTT_GRP_F_PRC.xlsx"
# excel_file = "ZGVTt_GRP_F_PRM.xlsx"
...
for _, row in df_mapping.iterrows():
    old_contract = row["SAMMELN_Source"]
    new_contract = row["SAMMELN_Clone"]
    ...
    # Στήλες προς συγκριση
    comparedCols = [c for c in df_data.columns if c!="SAMMELN"]

    merged = df_old.merge(
        df_new,
        how="outer",
        on=comparedCols,
        suffixes=("_Source", "_Clone"),
        indicator=True
    )
    
    # Προσθέτουμε στήλες με τα συμβόλαια για αναφορά
    merged["SAMMELN_Source"] = old_contract
    merged["SAMMELN_Clone"] = new_contract
